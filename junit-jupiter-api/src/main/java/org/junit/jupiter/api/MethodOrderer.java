...
public interface MethodOrderer {
	...
	class MethodNameReverse implements MethodOrderer {

		public MethodNameReverse() {
		}
		...
		private static final Comparator<MethodDescriptor> comparator = Comparator.<MethodDescriptor, String> comparing(
			descriptor -> descriptor.getMethod().getName(), (s1, s2) -> {
	...
	class DisplayNameReverse implements MethodOrderer {

		public DisplayNameReverse() {
		}
		...
		private static final Comparator<MethodDescriptor> comparator = Comparator.comparing(
			MethodDescriptor::getDisplayName, (s1, s2) -> {
				return s2.compareTo(s1);
			});
	}
